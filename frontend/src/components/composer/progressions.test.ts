...
describe('chordsFor', () => {
  it('builds the Pop progression in C major', () => {
    expect(chordsFor(key('C dúr'), [1, 5, 6, 4])).toEqual(['C', 'G', 'Am', 'F']);
  });

  it('builds I–V–vi–IV in A major', () => {
    expect(chordsFor(key('A dúr'), [1, 5, 6, 4])).toEqual(['A', 'E', 'F#m', 'D']);
  });

  it('builds the Andalusian progression in A minor and E minor', () => {
    expect(chordsFor(key('A moll'), [1, 6, 3, 7])).toEqual(['Am', 'F', 'C', 'G']);
    expect(chordsFor(key('E moll'), [1, 6, 3, 7])).toEqual(['Em', 'C', 'G', 'D']);
  });

  it('minor i–iv–v', () => {
    expect(chordsFor(key('A moll'), [1, 4, 5])).toEqual(['Am', 'Dm', 'Em']);
  });
});
